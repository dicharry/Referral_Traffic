...
DATE_TRUNC(DATE(SAFE_CAST(date_time as timestamp)), WEEK(MONDAY)) as week_start,
...
AND DATE(TIMESTAMP(CONCAT(SUBSTR(_TABLE_SUFFIX,1,4),'-',SUBSTR(_TABLE_SUFFIX,-2),'-','01'))) >= DATE_TRUNC(DATE_ADD(CURRENT_DATE(), INTERVAL -3 MONTH), MONTH)
...
DATE_TRUNC(DATE(SAFE_CAST(read_time as timestamp)), WEEK(MONDAY)) as week_start,
